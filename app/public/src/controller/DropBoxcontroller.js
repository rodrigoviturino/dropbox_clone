class DropBoxController {
  ...
  connectFirebase() {
    ...
    const firebaseConfig = {
      apiKey: "AIzaSyBGgrWZY7-smN1-OWsRklrmadneXoR1GE0",
      authDomain: "dropbox-clone-5c4a7.firebaseapp.com",
      databaseURL: "https://dropbox-clone-5c4a7.firebaseio.com",
      projectId: "dropbox-clone-5c4a7",
      storageBucket: "dropbox-clone-5c4a7.appspot.com",
      messagingSenderId: "642482507536",
      appId: "1:642482507536:web:064b474b6872c58e235f95",
      measurementId: "G-H329PYEY8E",
    };
    // Initialize Firebase
  ...
  initEvents() {
    this.listFilesEl.addEventListener("selectionchange", (event) => {
      console.log("selection", this.getSelection().length);
    ...
    this.inputFilesEl.addEventListener("change", (event) => {
      // Desabilitando botão de enviar arquivo quando estiver enviando arquivo
      this.btnSendFileEl.disabled = true;

      this.uploadTask(event.target.files)
        .then((responses) => {
          responses.forEach((resp) => {
            // pegamos o conteudo principal do arquivo de imagem, para salvar no firebase. firebase precisa dessa info filtrada para aceitar os campos
            // console.log(resp.files["input-file"]);
            // SET é para guardar os tais documentos no firebase
  ...
  uploadProgress(event, file) {
    ...
    let timeleft = (100 - porcent) * timespent;
    // console.log("123", timeleft);
        ...
        <g>
          <g>
            <g filter="url(#mc-content-unknown-large-a)">
              <path id="mc-content-unknown-large-b_2_" d="M47,30h66c2.209,0,4,1.791,4,4v92c0,2.209-1.791,4-4,4H47c-2.209,0-4-1.791-4-4V34 C43,31.791,44.791,30,47,30z"></path>
            </g>
            <g>
              <path id="mc-content-unknown-large-b_1_" fill="#F7F9FA" d="M47,30h66c2.209,0,4,1.791,4,4v92c0,2.209-1.791,4-4,4H47 c-2.209,0-4-1.791-4-4V34C43,31.791,44.791,30,47,30z"></path>
            </g>
          </g>
        </g>
        <g>
          <path fill-rule="evenodd" clip-rule="evenodd" fill="#848484" d="M81.148,62.638c8.086,0,16.173-0.001,24.259,0.001 c1.792,0,2.3,0.503,2.301,2.28c0.001,11.414,0.001,22.829,0,34.243c0,1.775-0.53,2.32-2.289,2.32 c-16.209,0.003-32.417,0.003-48.626,0c-1.775,0-2.317-0.542-2.318-2.306c-0.002-11.414-0.003-22.829,0-34.243 c0-1.769,0.532-2.294,2.306-2.294C64.903,62.637,73.026,62.638,81.148,62.638z M81.115,97.911c7.337,0,14.673-0.016,22.009,0.021 c0.856,0.005,1.045-0.238,1.042-1.062c-0.028-9.877-0.03-19.754,0.002-29.63c0.003-0.9-0.257-1.114-1.134-1.112 c-14.637,0.027-29.273,0.025-43.91,0.003c-0.801-0.001-1.09,0.141-1.086,1.033c0.036,9.913,0.036,19.826,0,29.738 c-0.003,0.878,0.268,1.03,1.069,1.027C66.443,97.898,73.779,97.911,81.115,97.911z"></path>
          <path fill-rule="evenodd" clip-rule="evenodd" fill="#848484" d="M77.737,85.036c3.505-2.455,7.213-4.083,11.161-5.165 c4.144-1.135,8.364-1.504,12.651-1.116c0.64,0.058,0.835,0.257,0.831,0.902c-0.024,5.191-0.024,10.381,0.001,15.572 c0.003,0.631-0.206,0.76-0.789,0.756c-3.688-0.024-7.375-0.009-11.062-0.018c-0.33-0.001-0.67,0.106-0.918-0.33 c-2.487-4.379-6.362-7.275-10.562-9.819C78.656,85.579,78.257,85.345,77.737,85.036z"></path>
          <path fill-rule="evenodd" clip-rule="evenodd" fill="#848484" d="M87.313,95.973c-0.538,0-0.815,0-1.094,0 c-8.477,0-16.953-0.012-25.43,0.021c-0.794,0.003-1.01-0.176-0.998-0.988c0.051-3.396,0.026-6.795,0.017-10.193 c-0.001-0.497-0.042-0.847,0.693-0.839c6.389,0.065,12.483,1.296,18.093,4.476C81.915,90.33,84.829,92.695,87.313,95.973z"></path>
          <path fill-rule="evenodd" clip-rule="evenodd" fill="#848484" d="M74.188,76.557c0.01,2.266-1.932,4.223-4.221,4.255 c-2.309,0.033-4.344-1.984-4.313-4.276c0.03-2.263,2.016-4.213,4.281-4.206C72.207,72.338,74.179,74.298,74.188,76.557z"></path>
          </g>
        </svg>
        `;
        break;
  ...
  readFiles() {
    ...
    this.getFirebaseRef().on("value", (snapshot) => {
      // zerando ele para quando recarregar, ele receber valores atuais
    ...
    li.addEventListener("click", (event) => {
      if (event.shiftKey) {
        ...
        if (firstLi) {
          ...
          lis.forEach((el, indx) => {
            if (indx >= index[0] && indx <= index[1]) {
              el.classList.add("selected");
            }
          });
          // retornando para ele parar e não proseguir, se não vai executar o toggle lá em baixodo selected
          return true;
        }
      }
